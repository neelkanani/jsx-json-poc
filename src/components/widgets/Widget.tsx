...
export default function Widget() {
  ...
  const addWidget = (widget: string) => {
    switch (widget) {
      case 'Container':
        const container = (
          <div className="flex border-1 border-amber-50 h-2 " onClick={() => onClick()}></div>
        )
        setWidgets([...widgets, container]);
        break;
      case 'Grid':
        const grid = (
          <div className="grid grid-cols-3 gap-4 border-1 border-amber-50 h-2" onClick={() => onClick()}>
            <div className='border-1 border-amber-600'></div>
            <div className='border-1 border-amber-600'></div>
            <div className='border-1 border-amber-600'></div>
  ...
  return (
    <div className="flex h-screen">
      ...
      <div className="flex-1 bg-white p-4 border-l border-r">
        <h2 className="text-lg font-bold mb-4">Drag widget here</h2>
        <div className="border-2 border-dashed p-10 text-center text-gray-500">
          {widgets.length === 0 ? (
            ...
            widgets.map((widget, index) => (
              <div 
                key={index} 
                className={`p-4 bg-gray-200 mb-2 rounded ${widget === 'Container' ? 'flex flex-col md:flex-row p-4 gap-4' : ''}`}
              >
                {widget}
              </div>
            ))
